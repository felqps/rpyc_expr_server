...
#--code--
    ...
    def insertFill(self, filrpt):
        if "fills" not in self.__dict__:
            self.fills = []
        self.fills.append(filrpt)
        self.fil_sz += filrpt.fil_sz
        
        self.todo_sz = self.ord_sz-self.fil_sz

    def getTrdPnl(self, lst_pri, factor=1):
        if "fills" not in self.__dict__:
            return 0
        self.trd_pnl = 0
        for filrpt in self.fills:
            self.trd_pnl += filrpt.getTrdPnl(lst_pri, factor)
        return self.trd_pnl
