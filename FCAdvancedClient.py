...
class FCAdvancedClient():
    def __init__(self, options):
        ...
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        # self.logger = Logger(f'{log_dir}/FCAdvancedClient_{QpsUtil.dt_suffix()}.log').logger
        self.logger = Logger(f'{log_dir}/FCAdvancedClient_{QpsUtil.dt_suffix()}.log').getFileLogger("FCAdvancedClient")
    ...
    def run_cmd_in_worker(self, job:dict, job_queue:"Queue"):
        ...
        socket.send_multipart([job["cmd"].encode('utf-8'), pickle.dumps({"hosts":job["hosts"], "heavy_score":job["heavy_score"], "timeout":job["timeout"]})])
        # job["status"] = "Running"
    ...
    def check_worker_available(self, hosts:list=[], heavy_score:int=0):
        ...
        socket.identity = get_identity(self.opt.priority).encode("utf-8")
        socket.connect(client_url)
        socket.send_multipart([b'CHECK_WORKER_AVAILABLE', pickle.dumps({"hosts":hosts, "heavy_score":heavy_score})])
        reply = socket.recv()
        try:
            result = int(reply)
        except Exception as err:
    ...
    def remove_all_jobs(self):
        ...
        socket.identity = get_identity(self.opt.priority).encode("utf-8")
        socket.connect(client_url)
        socket.send_multipart([b'REMOVE', b""])
        reply = socket.recv()
        try:
...
def list_cmds():
    cmds = [
        "python /Fairedge_dev/app_qpscloud/FCAdvancedClient.py --addr_cfg /qpsdata/config/FCScheduler.cfg --pool_size 10 /qpsdata/temp/ace.txt",
    ]
    print('\n'.join(cmds))
...
if __name__ == "__main__":
    ...
    fc_client = FCAdvancedClient(options)
