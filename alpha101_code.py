...
def ts_rank(df, window=10):
    ...
    """
    return (df.rolling(window).apply(rolling_rank)-1)/window
...
def decay_linear(df, period=10):
    ...
    #rc = pd.DataFrame(na_lwma, index=df.index, columns=['CLOSE']) 
    rc = pd.DataFrame(na_lwma, index=df.index, columns = df.columns)  
    # import sys
